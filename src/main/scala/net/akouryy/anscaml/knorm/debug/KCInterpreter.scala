...
class KCInterpreter {
  ...
  private[this] val output = new mutable.StringBuilder()
  ...
  private[this] val stdlib =
    Map[String, PartialFunction[List[Value], Value]](
      "$ext_print_char" -> { case List(VInt(i)) => output += i.toChar; VTuple(Nil) },
      ...
      "$ext_float_of_int" -> { case List(VInt(i)) => VFloat(i) },
      "$ext_bits_of_float" -> { case List(VFloat(f)) => VInt(java.lang.Float.floatToRawIntBits(f)) },
  ...
  private[this] def interpret(kc: KClosed, env: Map[ID, Value]): TailRec[Value] = {
    ...
    roughStep += 1
    if((roughStep & 8191) == 0) println(s"$roughStep ${env.size}")
  ...
  def apply(prog: KCProgram, input: FileInputStream): String = {
    this.input = input
    output.clear()
    gConsts.clear()
    roughStep = 0
    ...
    functions = prog.fDefs.map {
      case CFDef(Entry(name, _), args, _, body) =>
        name -> ((act: List[Value]) => {
          interpret(body, args.map(_.name).zip(act).toMap)
        })
    }.toMap
